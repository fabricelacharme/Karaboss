...
namespace Karaboss.xplorer
{
    ...
    #region FAB Delegate
    public delegate void ScmPlayEditEventHandler(object sender, FileInfo[] fInfos, bool bplay);
    public delegate void ScmAddToPlaylistEventHandler(object sender, FileInfo[] fInfos,  string plName, string key, bool bcreate);

    #endregion FAB Delegate
    ...
    public class ShellContextMenu : NativeWindow, IDisposable
    {
        ...
        private void ShowContextMenu(Point pointScreen)
        {
            ...
            try
            {
                ...
                pMenu = CreatePopupMenu();               // Main menu
                IntPtr plmenu = CreatePopupMenu();       // Playlist menu

                #region menu cascading "Add to playlist"      

                MENUITEMINFO itemInfo = new MENUITEMINFO(Strings.addToPlaylist);
                itemInfo.fMask = (MIIM.SUBMENU | MIIM.STRING);
                itemInfo.fType = MFT.STRING; // Force le type en chaîne de caractères
                itemInfo.hSubMenu = plmenu;

                int HRESULT = InsertMenuItem(pMenu, 0, true, ref itemInfo);
                ...
                if (m_PlGroup.Count > 0)
                {
                    ...
                    for (int i = 0; i < m_PlGroup.Count; i++)
                    {
                        ...
                        for (int j = 0; j < plgi.Playlists.Count; j++)
                        {
                            ...
                            InsertMenu(foldermenu, 1, (int)(MFT.BYPOSITION), (IntPtr)PLCMD, plName);                            
                            PLCMD++;
                        }
                        ...
                        if (ParentKey == null)
                        {
                            ...
                            itemInfo = new MENUITEMINFO(folder);
                            itemInfo.fMask = (MIIM.SUBMENU | MIIM.STRING);
                            itemInfo.fType = MFT.STRING; // Force le type en chaîne de caractères
                            itemInfo.hSubMenu = foldermenu;

                            // Insert the folder popup menu to plmenu                            
                            AppendMenu(plmenu, MFT.POPUP, (uint)foldermenu, folder);
                        ...
                        else
                        {
                            ...
                            IntPtr hmenu = (IntPtr)dicMenus.FirstOrDefault(t => t.Key == ParentKey).Value;

                            // Insert the folder popup menu to plmenu
                            HRESULT = InsertMenuItem(hmenu, 0, true, ref itemInfo);
                        }                        
                    }
                }                               
                ...
                // Add a separator
                AppendMenu(plmenu, MFT.SEPARATOR, 0, string.Empty);

                // Add to a new playlist
                plName = Strings.NewPlaylist;
                AppendMenu(plmenu, ichecked, PLCMD, plName);
                //PLCMD++;

                #endregion menu playlist items


                #region Menu play, edit
                InsertMenu(pMenu, 1, (int)(MFT.BYPOSITION), (IntPtr)100, Strings.play);
                InsertMenu(pMenu, 2, (int)(MFT.BYPOSITION), (IntPtr)101, Strings.edit);

                InsertMenu(pMenu, 3, (int)(MFT.BYPOSITION | MFT.SEPARATOR), (IntPtr)0, string.Empty);
                #endregion Menu play, edit
                ...
                if (nSelected != 0)
                {
                    // AIGUILLAGE DE VOS MENUS PERSONNALISÉS
                    if (nSelected == 100)
                    {
                        // L'utilisateur a cliqué sur "Play" !
                        // Déclenchez ici votre lecteur de fichier .kar
                        //System.Windows.Forms.MessageBox.Show("Action : Lancement de la lecture du morceau .kar !");
                        invokePlayEdit(true);
                    }
                    else if (nSelected == 101)
                    {
                        // L'utilisateur a cliqué sur "Edit" !
                        //System.Windows.Forms.MessageBox.Show("Action : Ouverture de l'éditeur !");
                        invokePlayEdit(false);
                    }
                    else if (nSelected >= 1000 && nSelected < PLCMD)
                    {
                        // L'utilisateur a cliqué sur une playlist existante
                        uint indexPlaylist = nSelected - 1000;
                        ...
                        invokeAddToPlaylist(plName, Key, false);
                    }
                    else if (nSelected == PLCMD)
                    {
                        uint indexPlaylist = nSelected - 1000;
                        // L'utilisateur a cliqué sur une nouvelle playlist
                        invokeAddToPlaylist(string.Empty, string.Empty, true);
                    }
                    else
                    {
        ...
        #region FAB Application Karaboss      

        public event ScmPlayEditEventHandler PlayEditFile;
        public event ScmAddToPlaylistEventHandler AddToPlaylist;
        ...
        {
            PlayEditFile?.Invoke(this, fInfos, bplay);           
        }

        private void invokeAddToPlaylist(string plName, string key, bool bcreate)
        {                        
            AddToPlaylist?.Invoke(this, fInfos, plName, key, bcreate);
        }


        #endregion FAB Application Karaboss
        ...
        private static extern int InsertMenuItem(IntPtr hMenu, uint uItem, bool fByPosition, ref MENUITEMINFO lpmii);

        [DllImport("user32.dll", CharSet = CharSet.Auto)]
        private static extern bool AppendMenu(IntPtr hMenu, MFT uFlags, uint uIDNewItem, string lpNewItem);
    ...
    internal static class ShellHelper
    {
        ...
        {
            // https://stackoverflow.com/questions/53962324/system-overflowexception-when-casting-a-system-intptr-to-a-uint
