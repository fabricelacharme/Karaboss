...
using Un4seen.Bass;
using static Karaboss.MP3.Config;
using PrgAutoUpdater;
using static Karaboss.MP3.MusicStream;
    ...
    public class BassAudioEngine : IPlayer
    {
                ...
                case Action.ActionType.ACTION_MUSIC_FORWARD:
                    {
                        if (g_Player.IsMusic)
                        {
                            if (Config.UseSkipSteps)
                            {
                                ...
                                LogInfo(string.Format("BASS: Skipping in stream for {0}", strStatus);
                            }
                            else
                            {
                                LogInfo(string.Format("BASS: Playback speed {0}", g_Player.Speed));
                            }
                        }
                        break;
                    }
                ...
                case Action.ActionType.ACTION_TOGGLE_MUSIC_GAP:
                    {
                        ...
                        LogInfo(string.Format("BASS: Playback changed to {0}", type));
                        break;
                    }
            }
        }
        */
        ...
        private void CommandThread()
        {
                ...
                while (!exitThread)
                {
                    ...
                    lock (_commandQueueSync)
                    {
                        ...
                        switch ((int)item.cmd)
                        {
                            ...
                            default:
                                LogError(string.Format("BASS: CommandThread unknown command {0}", (int)item.cmd));
                                continue;
                        }
                    }
                }
            ...
            {
                LogError(string.Format("BASS: CommandThread exception {0}", ex));
            }
        }
        ...
        private void StopCommand()
        {
            lock (_syncRoot)
            {
                ...
                try
                {
                    ...
                    {
                        LogDebug(string.Format("BASS: Stop of stream {0}.", stream.FilePath));
                        if (Config.SoftStop && !stream.IsDisposed && !stream.IsCrossFading)
                        {
                            if (Config.CrossFadeIntervalMs > 0)
                            {
                                LogDebug(string.Format("BASS: Performing Softstop of {0}", stream.FilePath));
                ...
                {
                    LogError(string.Format("BASS: Stop command caused an exception - {0}. {1}", ex.Message, ex.StackTrace));
                }
        ...
        private void Initialize()
        {
            ...
            try
            {
                LogInfo(string.Format("BASS: Initialize BASS environment ..."));
                ...
                {
                    LogDebug(string.Format("BASS: Create Stream copy for Visualisation"));
                    _streamcopy = new StreamCopy(this);
                    _streamcopy.StreamCopyFlags = BASSFlag.BASS_SAMPLE_FLOAT | BASSFlag.BASS_STREAM_DECODE;
                }
                LogInfo(string.Format("BASS: Initializing BASS environment done."));
            ...
            {
                LogError(string.Format("BASS: Initialize thread failed.  Reason: {0}", ex.Message));
            }
        }
        ...
        public void InitBass()
        {
            try
            {
                LogInfo(string.Format("BASS: Initializing BASS audio engine..."));
                bool initOK = false;

                if (_bassFreed && Config.MusicPlayer != AudioPlayer.Bass)
                {
                    LogDebug(string.Format("BASS: BASS audio engine was previously freed. Re-Init"));
                ...
                if (initOK)
                {
                    ...
                    LogInfo(string.Format("BASS: Initialization done."));
                    _initialized = true;
                    _bassFreed = false;
                }
                else
                {
                    ...
                    LogError(string.Format("BASS: Error initializing BASS audio engine {0}", Enum.GetName(typeof(BASSError), error)));
                }
            }
            catch (Exception ex)
            {
                LogError(string.Format("BASS: Initialize failed. Reason: {0}", ex.Message));
            }
        }
        ...
        private bool InitDirectSoundDevice()
        {
            ...
            if (info != null)
            {
                ...
                LogInfo(string.Format("BASS: Device Information"));
                LogInfo(string.Format("BASS: ---------------------------------------------"));
                LogInfo(string.Format("BASS: Name: {0}", info.ToString()));
                LogInfo(string.Format("BASS: Directsound version: {0}", info.dsver));
                LogInfo(string.Format("BASS: # Output Channels: {0}", info.speakers));
                LogDebug(string.Format("BASS: Minimum Buffer Samples: {0}", info.minbuf));
                LogDebug(string.Format("BASS: Current Sample rate: {0}", info.freq));
                LogDebug(string.Format("BASS: Maximum Sample rate: {0}", info.maxrate));
                LogDebug(string.Format("BASS: Minimum Sample rate: {0}", info.minrate));
                LogInfo(string.Format("BASS: ---------------------------------------------"));
            }
        ...
        private int GetSoundDevice()
        {
            ...
            {
                LogInfo(string.Format("BASS: Using default Sound Device"));
                sounddevice = -1;
            }
            else
            {
                ...
                if (!foundDevice)
                {
                    LogWarn("BASS: specified Sound device does not exist. Using default Sound Device");
                    sounddevice = -1;
                }
                else
                {
                    LogInfo(string.Format("BASS: Device Information"));
                    LogInfo(string.Format("BASS: ---------------------------------------------"));
                    LogInfo(string.Format("BASS: Name: {0}", soundDeviceDescriptions[sounddevice].name));
                    LogInfo(string.Format("BASS: Default Device: {0}", soundDeviceDescriptions[sounddevice].IsDefault.ToString()));
                    LogInfo(string.Format("BASS: ---------------------------------------------"));
                }
            }
            return sounddevice;
        ...
        public void DisposeAndCleanUp()
        {
            ...
            catch (DllNotFoundException ex)
            {
                ...
                if (!(IntPtr.Size > 4 && ex.Message.IndexOf("bass_wadsp", StringComparison.OrdinalIgnoreCase) >= 0))
                    LogError(string.Format("BassAudioEngine: DisposeAndCleanUp {0}", ex.Message));
            }
            catch (Exception ex)
            {
                LogError(string.Format("BassAudioEngine: DisposeAndCleanUp {0}", ex.Message));
        ...
        public void FreeBass()
        {
            ...
            lock (_syncRoot)
            {
                if (!_bassFreed)
                {
                    LogInfo(string.Format("BASS: Freeing BASS. Non-audio media playback requested."));
        ...
        {
            LogError(string.Format("BASS: {0}() failed: {1}", methodName, Enum.GetName(typeof(BASSError), Bass.BASS_ErrorGetCode())));
        }
        ...
        private bool PlayInternal(string filePath)
        {
            ...
            */
            ...
            if (_mixer == null)
            {
                ...
                {
                    LogError(string.Format("BASS: Could not create Mixer. Aborting playback."));
                    return false;
                }
            ...
            else
            {
                ...
                {
                    LogDebug(string.Format("BASS: New stream has different number of channels or sample rate. Need a new mixer."));
                    // Free Mixer
                    _mixer.Dispose();
                    _mixer = null;
                    _mixer = new MixerStream(this);
        ...
        public override bool Play(string filePath)
        {
            ...
            try
            {
                ...
                {
                    LogInfo(string.Format("BASS: playback started"));
                ...
                {
                    LogError(string.Format("BASS: Unable to play {0}.  Reason: {1}.", filePath,
            ...
            {
                result = false;
                LogError(string.Format("BASS: Play caused an exception:  {0}.", ex));
            }
            ...
            try
            {
                ...
                if (oldPlayState == PlayState.Paused)
                {
                    ...
                    if (stream.Filetype.FileMainType == FileMainType.WebStream && !g_Player.ForcePauseWebStream) // in case of UPnP Renderer we want support pause
                    {
                        ...
                        LogDebug(string.Format("BASS: Stopping and Restarting Webstream {0}", stream.FilePath));
                        g_Player.OnStopped();
                        Stop();
                        Play(filePath);
                        return;
                    ...
                    LogDebug(string.Format("BASS: Resuming stream {0}", stream.FilePath));
                    _state = PlayState.Playing;
                ...
                {
                    LogDebug(string.Format("BASS: Pausing stream {0}", stream.FilePath));
        ...
        public override void Stop()
        {
            lock (_syncRoot)
            {
                ...
                if (_mixer == null)
                {
                    LogDebug(string.Format("BASS: Already stopped. Don't execute Stop a second time"));
                    return;
                }
                */
                lock (_commandQueueSync)
        ...
        public bool SeekForward(int ms)
        {
            if (_speed == 1) // not to exhaust log when ff
                LogDebug(string.Format("BASS: SeekForward for {0} ms", Convert.ToString(ms)));
        ...
        public bool SeekReverse(int ms)
        {
            if (_speed == 1) // not to exhaust log
                LogDebug(string.Format("BASS: SeekReverse for {0} ms", Convert.ToString(ms)));
        ...
        {
            LogDebug(string.Format("BASS: SeekToTimePosition: {0} ", Convert.ToString(position)));
        ...
        public MusicTag GetStreamTags()
        {
            ...
            {
                LogError(string.Format("BASS:GetStreamTags: {0}", ex.Message));
                tag.Year = 0;
            }
            return tag;
        }
        ...
        public void SwitchToGaplessPlaybackMode()
        {
            if (_playBackType == (int)PlayBackType.CROSSFADE)
            {
                ...
                LogInfo(string.Format("BASS: Playback of complete Album starting. Switching playbacktype from {0} to {1}",
        ...
        public void SwitchToDefaultPlaybackMode()
        {
            if (_savedPlayBackType > -1)
            {
                LogInfo(string.Format("BASS: Playback of complete Album stopped. Switching playbacktype from {0} to {1}",
                         Enum.GetName(typeof(PlayBackType), _playBackType),
        ...
        public bool GetSpectrum(ref List<int> spectrum, byte lines, int _min, int _max)
        {
            ...
            {
                LogDebug(string.Format("BASS: GetSpectrum(): {0}", ex.Message));
                return false;
            }
            if (_result < 0)
            {
        ...
        private void LogWarn(string v, string message = null)
        {
            Console.WriteLine(string.Format(v, message));
        }   
