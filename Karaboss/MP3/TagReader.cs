    ...
    public class TagReader
    {
        ...
        public static MusicTag ReadTag(string strFile)
        {
            ...
            if (CueUtil.isCueFakeTrackFile(strFile))
            {
                ...
                {
                    LogWarn(string.Format("TagReader: Exception reading file {0}. {1}", strFile, ex.Message));
                }
            }
            ...
            try
            {
                ...
                if (tag == null)
                {
                    LogWarn("Tagreader: No tag in file - {0}", strFile);
                    return null;
                }
                        ...
                        if (!foundPopm)
                        {
                            ...
                            if (apetag != null)
                            {
                                ...
                                if (apeItem != null)
                                {
                                    ...
                                    {
                                        musictag.Rating = 0;
                                        LogWarn(string.Format("Tagreader: Unsupported APE rating format - {0} in {1} {2}", rating, strFile, ex.Message));
                                    }
                                }
                            }
                        }
                ...
                else if (tag.MimeType == "taglib/ogg" || tag.MimeType == "taglib/flac")
                {
                    ...
                    if (xiph != null)
                    {
                        ...
                        if (rating.Length > 0)
                        {
                            ...
                            {
                                LogError("TagReader:ReadTag {0}", ex.Message);
                            }
                        }
                    }
                }
                ...
                if (musictag.Title == "")
                {
                    LogWarn("TagReader: Empty Title found in file: {0}. Please retag.", strFile);
            ...
            {
                LogWarn(string.Format("Tagreader: Unsupported File Format {0} {1}", strFile, ex.Message));
            }
            catch (Exception ex)
            {
                LogWarn(string.Format( "TagReader: Exception reading file {0}. {1}", strFile, ex.Message));
        ...
        #region Logs
        private static void LogWarn(string format, string message = null)
        {
            Console.WriteLine(format, message);
        }

        private static void LogError(string format, string message = null)
        {
            Console.WriteLine(format, message);
        }
        ...
        public static bool WriteLyrics(string strFile, string strLyrics)
        {
            ...
            {
                LogWarn(string.Format("TagReader: Exception writing file {0}. {1}", strFile, ex.Message));
            }
            return false;
        }
