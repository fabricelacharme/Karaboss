    ...
    public class Mp3Player
    {
        ...
        public string FileName
        {
            ...
            set
            {
                ...
                if (mBassInitalized)
                {
                    if (IsCloudFile(_FileName))
                    {
                        // Si c'est un fichier OneDrive, on charge en arrière-plan sans bloquer l'UI
                        Task.Run(() => Load(_FileName));
                    }
                    else
                    {
                        // Fichier local : chargement instantané synchrone habituel
                        Load(_FileName);
                    }
                }
            }
