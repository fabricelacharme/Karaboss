...
namespace Sanford.Multimedia.Midi.Score
{
    ...
    public class ChordSymbol : MusicSymbol
    {
        ...
        public override void Draw(Graphics g, Pen pen, int ytop, int tracknum, Rectangle selrect, int OffsetX)
        {
            ...
            int xpos = DrawAccid(g, pen, ytop, tracknum, selrect, OffsetX);

            /* Draw the notes */
            g.TranslateTransform(xpos, 0);
            DrawNotes(g, pen, ytop, topstaff, tracknum, selrect, OffsetX);
         ...
         */
        public int DrawAccid(Graphics g, Pen pen, int ytop, int tracknum, Rectangle selrect, int OffsetX)
        {
            int xpos = 0;
            ...
            foreach (AccidSymbol symbol in accidsymbols)
            {
                ...
                g.TranslateTransform(xpos, 0);
                symbol.Draw(g, pen, ytop, tracknum, selrect, OffsetX);
                g.TranslateTransform(-xpos, 0);
                prev = symbol;
            }
            if (prev != null)
            ...
            foreach (NoteData note in notedata)
            {
                ...
                if (sheetmusic.bSelectingNote == true)
                {
                    ...
                    if ((OffsetX >= selRect.X) && (OffsetX < selRect.X + selRect.Width) && (OffsetY >= selRect.Y) && (OffsetY <= selRect.Y + selRect.Height))
                    {
                        ...
                        sheetmusic.AddSelectedNote(notes[note.midinoteindex]);
                    }
                }
                else
                {
