...
namespace FlShell
{
    ...
    public class ShellView : Control, INotifyPropertyChanged
    {
        ...
        //private string s_file;  // fullpath of selected file on double click
       ...
       /// <summary>
        ...
        public void Navigate(ShellItem folder)
        {
            ...
            if (!e.Cancel)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Console.Write("Erreur dans module ShellView.cs / Navigate" + ex.Message);
        ...
        public void SetColumnWidth(int column, int width)
        {
            ...
            // FAB - to set the width of the first column (nam of file)
        ...
        public ShellItem[] SelectedItems
        {
            get
            {
                ...
                /*
                 * Atention gros bug, ne jamais appeler shellView.SelectedItems dans la même procédure 
                 * Contournement : travailler avec une copie
                 * Calculer FlShell.ShellItem[] fls = shellView.SelectedItems;
                 * puis n'utiliser que la copie fls dans le reste de la procédure
                 * 
                 */               
                ...
                if (selection != null)
                {
                    ...
                    if (selection.QueryGetData(ref format) == 0)
                    {
                        ...
                        // FAB 29/06/2016
                        //if (itemCount < 0 || itemCount > int.MaxValue)
                        ...
                        try
                        {
                            ...
                            {
                                int offset = Marshal.ReadInt32(storage.unionmember, 8 + (n * 4));

                                // FAB 29/06/16 : remove (int) cast
                                //result[n] = new ShellItem(m_CurrentFolder, (IntPtr)((int)storage.unionmember + offset));
                                result[n] = new ShellItem(m_CurrentFolder, (IntPtr)(storage.unionmember + offset));
                        ...
                        catch (Exception ex)
                        {
                            ...
                            Console.Write("Erreur dans module ShellView.cs / public ShellItem[] SelectedItems\n" + ex.Message + "\n");
                        }
        ...
        /// <summary>
        /// FAB : Occurs when user double click
        /// </summary>
        //public event LanchAppEventHandler DoubleClicked;
        ...
        public override bool PreProcessMessage(ref Message msg)
        {
            ...
            if ((msg.Msg == WM_KEYDOWN) || (msg.Msg == WM_KEYUP))
            {
                switch (keyData)
                {
                    case Keys.Control:
                        if (keyCode == Keys.N)
                            lvFunctionKeyClicked(this, keyCode, keyData);
                     break;
                }
                ...
                switch (keyCode)
                {
                    ...
                    case Keys.F4:
                    case Keys.F6:
                        lvFunctionKeyClicked(this, keyCode, keyData); //F3 rename all, F4, replace all (Karaboss function keys)
                        return true;
                    case Keys.F5:
                        RefreshContents();
                        return true;
                    case Keys.Delete:
                        DeleteSelectedItems();  // Problem to solve: when delete is pressed during rename edition...
                        return true;
                }
            } 
        ...
        void CreateShellView()
        {
            ...
            {
                if (previous != null)
                    m_ComInterface.CreateViewWindow(previous, ref folderSettings, GetShellBrowser(), ref bounds, out m_ShellViewWindow);

                // FAB : Force width value of column 0 
                SetColumnWidth(0, lvfilenamecolumn);
    ...
    // FAB: F3, F4, F6
    public delegate void lvFunctionKeyEventHandler(object sender, Keys keyCode, Keys KeyData);
